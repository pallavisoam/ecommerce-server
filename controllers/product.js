...
const handleStar = (req, res, stars) => {
	Product.aggregate([
		{
			$project: {
				document: '$$ROOT', // this has now accessed all the fields of the product model otherwise we had to write all the fields manually so root is the shortcut .
				floorAverage: {
		...
		.exec((err, aggregates) => {
			...
			Product.find({ _id: aggregates })
				...
				.populate('postedBy', '_id name')
				.exec((err, products) => {
					if (err) console.log('PRODUCT AGGREGATE ERRROR', err);
					res.json(products);
				});
		});
};
